...
class DQNAgent(Agent):
    ...
    def learnNN(self, env, masked = False, n_episodes = 1500, n_save = 10000, trainingName = ""):
        l_epsilon = []
        l_win = []
        sum_win = 0
        ...
        for episode in tqdm.tqdm(range(n_episodes)):
            ...
            while not done:      
                ...
                if masked:
                    ...
                    action = self.pickActionMaybeMasked(env,state)  
...
class DoubleDQNAgent(Agent):
    def __init__(self, player = 1, env = gym.make('LunarLander-v2'), loading=True, name = ""):
        ...
        self.target_q = DQNetwork(state_len, 81, learning_rate)  
    ...
    def learn(self, error):
        ...
        online_actions = self.q.forward(new_states_batch).argmax(axis=1)
        target_q_values = self.target_q.forward(new_states_batch)
        Q_value_next_state = target_q_values.gather(1, online_actions.unsqueeze(1)).squeeze(1)

        Q_target = rewards_batch + self.gamma * Q_value_next_state * (1 - dones_batch)
        prediction = self.q.forward(states_batch).gather(1, actions_batch.unsqueeze(1)).squeeze(1)

        loss = self.q.loss(prediction, Q_target)
        loss.backward()
        ...
        self.it_counter += 1
        if self.it_counter % 100 == 0:  # Update every 1000 steps
            self.target_q.load_state_dict(self.q.state_dict())
        return
    ...
    def learnNN(self, env, masked = False, n_episodes = 1500, n_save = 10000, trainingName = ""):
        ...
        for episode in tqdm.tqdm(range(n_episodes)):
            ...
            while not done: 
                ...
                new_state,reward, done, error = env.step(action) 
                score += reward  
                self.replay_buffer.store_transition(processObs(state), action, reward, processObs(new_state), done) 
                self.learn(error)
                state = new_state
